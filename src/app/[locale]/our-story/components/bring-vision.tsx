...
export const BringVision = () => {
  const t = useTranslations('ourStory.bring-vision');
    ...
    <section className="flex gap-10 px-[60px] py-[100px] max-md:flex-col max-md:px-4 max-md:py-[60px]">
      <section className="flex w-1/2 flex-col max-md:w-full">
        <div className="flex flex-col gap-4">
          <Title color="white">
            {t('title', {
              fallback: 'You Bring the Vision. We Bring the Framework.',
            })}
          </Title>
          <Text color="white">
            {t('description', {
              fallback: 'Together, we build something that lasts.',
            })}
          </Text>
        </div>
      ...
      <section className="flex w-1/2 flex-col gap-2 max-md:w-full">
        <Card
          value={t('card.0', {
            fallback: 'See How We Can Help You',
          })}
          url="/how-we-can-help-you"
        />
        <Card
          value={t('card.1', {
            fallback: 'Jump to What to Expect',
          })}
          url="/what-to-expect"
        />
      </section>
