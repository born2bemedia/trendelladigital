...
export const SpecificImpact = () => {
  const t = useTranslations('howWeCanHelpYou.specificImpact');

  const tb = useTranslations('detailedConsultings.business');
  const tm = useTranslations('detailedConsultings.marketing');

  const businessConsulting = getBusinessConsultingDetailed(tb);
  const marketingConsulting = getMarketingConsultingDetailed(tm);

  return (
    <section className="mx-4 flex flex-col gap-10 bg-white px-[60px] py-[100px] max-md:px-4 max-md:py-[60px]">
      <section className="flex flex-col gap-2">
        <Title color="black">
          {t('title', { fallback: 'Specific, High-Impact Areas of Support' })}
        </Title>
        <Text color="black">
          {t('description', {
            fallback:
              'Advisory without implementation is incomplete. We co-create with you, combining strategic planning with operational precision.',
          })}
        </Text>
      </section>
